...
def stencil(data, f, width):
    ...
    """
    stencil = []
    length = len(data) - (width - 1)
    for i in range(0, length):

        list = data[i:i + width]
        stencil.append(f(list))
...
def createBox(box):
    ...
    def boxFilter(l):
        ...
        boxLength = len(box)
        if len(l) != boxLength:
            print("Calling box filter with the wrong length list. Expect list of length should be {}.".format(boxLength))

        prod = [i[0] * i[1] for i in zip(box, l)]
        return float(sum(prod))
