...
def getFormattedText(filename) :
    ...
    lines = ['__' + x.strip().lower() + '__' for x in myFile.readlines()]
    return lines
...
def getDict(filename):
    #fill in
    nGramDict = {}
    fGramDict = getFormattedText(filename)
    for lines in fGramDict:
        for n in getNgrams(lines):
            if n in nGramDict.keys():
                nGramDict[n] += 1
            else:
                nGramDict[n] = 1
    return nGramDict
...
def topNCommon(filename,N):
    commonN = []
    myFile = getDict(filename)
    sorted_file = {x: y for x, y in sorted(myFile.items(), key=lambda item: item[1])}
    commonN = [sorted_file.popitem() for n in range(N)]
    return commonN
...
def getAllDicts(fileNamesList):
    langDicts = []
    for n in fileNamesList:
        langDicts.append(getDict(n))
...
def dictUnion(listOfDicts):
    ...
    unionNGrams = sorted(list(unionSet))
...
def compareLang(testFile,langFiles,N):
    ...
    commonN = []
    commonN = topNCommon(testFile, N)

    for i in range(len(commonN)):
        commonN[i] = commonN[i][0]
    list1_set = set(commonN)

    max = 0

    for file in langFiles:
        commonNlist = []
        commonNlist = topNCommon(file, N)

        for x in range(len(commonNlist)):
            commonNlist[x] = commonNlist[x][0]

        calc = set(commonNlist)
        set1 = list1_set.intersection(calc)
